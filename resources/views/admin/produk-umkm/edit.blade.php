...
<div class="p-6">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Edit Produk UMKM</h1>
                <p class="text-slate-600 mt-1">Edit produk: {{ $produk->nama }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200">
        <form action="{{ route('admin.produk-umkm.update', $produk) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="border-b border-slate-200 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="user_id" id="user_id" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('user_id') border-red-500 @enderror" required>
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ (old('user_id', $produk->user_id) == $user->id) ? 'selected' : '' }}>{{ $user->name }}</option>
                            @endforeach
                        </select>
                        @error('user_id')
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" id="nama" value="{{ old('nama', $produk->nama) }}" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('nama') border-red-500 @enderror" required>
                        @error('nama')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="kategori" id="kategori" value="{{ old('kategori', $produk->kategori) }}" placeholder="Contoh: Makanan, Kerajinan, Pertanian" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('kategori') border-red-500 @enderror" required>
                        @error('kategori')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="kontak" id="kontak" value="{{ old('kontak', $produk->kontak) }}" placeholder="Nomor HP atau WhatsApp" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('kontak') border-red-500 @enderror" required>
                        @error('kontak')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="harga" id="harga" value="{{ old('harga', $produk->harga) }}" min="0" step="0.01" class="w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('harga') border-red-500 @enderror" required>
                        </div>
                        @error('harga')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="stok" id="stok" value="{{ old('stok', $produk->stok) }}" min="0" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('stok') border-red-500 @enderror" required>
                        @error('stok')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('deskripsi') border-red-500 @enderror" placeholder="Jelaskan produk Anda secara detail..." required>{{ old('deskripsi', $produk->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="border-b border-slate-200 pb-6">
                <h3 class="text-lg font-semibold text-slate-800 mb-4">Foto Saat Ini</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @foreach($produk->foto as $index => $foto)
                    <div class="relative group">
                        <img src="{{ asset('storage/' . $foto) }}" alt="Foto Produk" class="w-full h-24 object-cover rounded-lg border border-slate-200">
                        <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg flex items-center justify-center">
                            <button type="button" onclick="removeExistingPhoto('{{ $foto }}', this)" class="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600">
                                Hapus
                            </button>
                        </div>
                        @if($index === 0)
                        <span class="absolute top-1 left-1 bg-blue-500 text-white text-xs px-2 py-1 rounded">Utama</span>
                        @endif
                    </div>
                    @endforeach
                </div>
                <input type="hidden" name="existing_photos" id="existing_photos" value="{{ json_encode($produk->foto) }}">
            </div>
            @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="status" id="status" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('status') border-red-500 @enderror" required>
                            ...
                            <option value="inactive" {{ old('status', $produk->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex items-center">
                        <div class="flex items-center h-5">
                            <input id="is_featured" name="is_featured" type="checkbox" value="1" {{ old('is_featured', $produk->is_featured) ? 'checked' : '' }} class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-slate-300 rounded">
                        </div>
...
function removeExistingPhoto(photoPath, button) {
    if (confirm('Apakah Anda yakin ingin menghapus foto ini?')) {
        ...
        existingPhotos = existingPhotos.filter(photo => photo !== photoPath);
        existingPhotosInput.value = JSON.stringify(existingPhotos);
