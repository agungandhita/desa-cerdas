...
<div class="p-6">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Edit APB Desa</h1>
                <p class="text-slate-600 mt-1">Edit data anggaran pendapatan dan belanja desa</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200">
        ...
        <form action="{{ route('admin.apbdes.update', $apbdes) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="tahun" id="tahun" required
                        ...
                        @for($year = 2020; $year <= 2030; $year++)
                            <option value="{{ $year }}" {{ (old('tahun') ?? $apbdes->tahun) == $year ? 'selected' : '' }}>
                                {{ $year }}
                            </option>
                        @endfor
                    </select>
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="approved" {{ (old('status') ?? $apbdes->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                        <option value="executed" {{ (old('status') ?? $apbdes->status) == 'executed' ? 'selected' : '' }}>Executed</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="bidang" id="bidang" required
                           value="{{ old('bidang') ?? $apbdes->bidang }}"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="jumlah_anggaran" id="jumlah_anggaran" required
                               value="{{ old('jumlah_anggaran') ?? $apbdes->jumlah_anggaran }}"
                               placeholder="0"
                               min="0"
                               step="0.01"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="realisasi" id="realisasi"
                               value="{{ old('realisasi') ?? $apbdes->realisasi }}"
                               placeholder="0"
                               min="0"
                               step="0.01"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                              ...
                              class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi') ?? $apbdes->deskripsi }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
